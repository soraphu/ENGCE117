...
int main() {
    int SalMN[ 20 ], count = 0, Max = 0, leng = 0, imax ;
    ...
    while( 1 ) { //วนลูปรับค่า
        ...
        scanf( "%d", &SalMN[ count ] ) ; getchar() ;
        sum += (float)SalMN[ count ] ;
        if( Max < SalMN[ count ] ) { //หาคนที่เงินเดือนมากสุด
            Max = SalMN[ count ] ;
            imax = count ;
        }//end if
        count ++ ;
    ...
    printf( " Bath\n" ) ;
    printf( "Most tax in company : %s (", name[ imax ] ) ;
    Setcomm( leng, str, ( SalMN[ imax ] * 12 ) * 0.07 ) ;
